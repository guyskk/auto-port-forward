...
- **配置**：TOML（`~/Library/Application Support/autoportforward/config.toml`）
...
```
~/Library/Application Support/autoportforward/config.toml
```
...
3. `lsof -nP -iTCP -sTCP:LISTEN | grep autoportforward` 能看到对应本地监听
