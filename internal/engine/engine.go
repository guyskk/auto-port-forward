...
// ClientHandle 抽象 SSH 客户端的全部能力：连接、关闭、远端执行、远端端口转发。
// sshctl.Client 直接满足；测试中用 fake 替代。
type ClientHandle interface {
	Connect(ctx context.Context) error
	Close() error
	Run(ctx context.Context, cmd string) ([]byte, error)
	AddForward(ctx context.Context, port int) error
	CancelForward(ctx context.Context, port int) error
...
type Deps struct {
	ClientFactory func(host sshcfg.Host) ClientHandle
	LocalScan     func(ctx context.Context) ([]model.LocalPort, error)
	Now           func() time.Time
	IsRoot        bool

	// Backoff 重连退避参数。零值用 sshctl.DefaultBackoff()。
...
type Engine struct {
	cfg   config.Config
	hosts []sshcfg.Host // 启用监控的 host 列表（alias 唯一）
	emit  events.Emitter
	deps  Deps
...
// serverState 每个 host 的运行态：ssh client、远端扫描缓存、当前 forward 映射。
type serverState struct {
	...
	cancel   context.CancelFunc     // 关闭本 host 的 connectLoop
}
...
// spawnServer 启动一个 host 的 connectLoop 并注册到 e.servers。
// 必须在持有 e.mu 时调用。
func (e *Engine) spawnServer(ctx context.Context, h sshcfg.Host) {
	sctx, cancel := context.WithCancel(ctx)
	st := &serverState{
		cfg:      h,
		client:   e.deps.ClientFactory(h),
		scanner:  scan.NewRemoteScanner(),
		forwards: map[int]*forwardHandle{},
		cancel:   cancel,
	}
	e.servers[h.Alias] = st
	e.wg.Add(1)
	go e.connectLoop(sctx, st)
}

// connectLoop 维护一个 host 的 ssh 连接：失败按 backoff 重试、断开后自动重连、
...
func (e *Engine) connectLoop(ctx context.Context, st *serverState) {
	...
	for {
		...
		e.emitServerStatus(ctx, st.cfg.Alias, "dialing", attempt, 0, "")
		err := st.client.Connect(ctx)
		if err == nil {
			attempt = 0
			downSince = time.Time{}
			e.emitServerStatus(ctx, st.cfg.Alias, "connected", 0, 0, "")
			select {
			case <-ctx.Done():
				return
			case <-st.client.Done():
				downSince = now()
				e.emitServerStatus(ctx, st.cfg.Alias, "broken", 0, 0, "connection lost")
				continue
			}
		}
		...
		state := "broken"
		if sshctl.IsDegraded(backoff, sinceDown) {
			state = "degraded"
		}
		e.emitServerStatus(ctx, st.cfg.Alias, state, attempt, sinceDown.Milliseconds(), err.Error())

		delay := sshctl.NextDelay(backoff, attempt)
		attempt++
...
// StopAll 停止扫描循环、关闭所有 forward、关闭 ssh client。
